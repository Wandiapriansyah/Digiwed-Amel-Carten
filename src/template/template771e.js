...
var AudioManager = {
    ...
    bindEvents: function() {
        ...
        $(document).on('click', '.drive-play-btn', function () {
    const wrapper = this.closest('.drive-video-wrapper');
    const iframe  = wrapper.querySelector('iframe');

    // 1. Pause background music
    if (window.AudioManager && AudioManager.isPlaying()) {
        AudioManager.pause();
    }

    // 2. Reload iframe dengan autoplay
...
const DRIVE_VIDEO_DURATION = 65000; // 1 menit 5 detik

$(document).on('click', '.drive-play-btn', function () {

    // pause musik kalau sedang main
    if (AudioManager && AudioManager.isPlaying()) {
        AudioManager.pause();
    }

    // resume musik setelah video selesai (1:05)
    setTimeout(() => {
        if (AudioManager && AudioManager.isUnlocked()) {
            AudioManager.play();
        }
    }, DRIVE_VIDEO_DURATION);
});
...
player.on('error', function () {
    ...
    $(video).html(`
        <div class="video-error-fallback">
            <a href="${url}" target="_blank" class="video-error-link">
